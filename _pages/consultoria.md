---
layout: page
permalink: /consultoria/
title: Consultoria
description: Serviços oferecidos
nav: true
nav_order: 3
calendar: true
---

Em um projeto de pesquisa científica, a parte estatística pode, facilmente, ser a maior dor de cabeça. Muitos cálculos, muitos conceitos, muitos detalhes. 

Mas nem tudo está perdido. Entre em contato comigo através do meu <a href="mailto:hektor.brasil@gmail.com">email</a> ou <a href="https://wa.me/5541991777188">WhatsApp<> e vamos conversar sobre como resolver seu problema.

<div class="grid-container">
    <div class="card">
        <div class="card-header">
            Tipos de trabalho
        </div>
        <div class="card-body">
            <ul>
                <li>TCC</li>
                <li>Dissertações</li>
                <li>Teses</li>
                <li>Artigos</li>
                <li>Iniciação científica</li>
            </ul>
        </div>
    </div>
    <div>
    <div class="card">
        <div class="card-header">
            Catálogo de Técnicas Estatísticas
        </div>
        <div class="card-body">
            <ul>
                <li>Análise descritiva</li>
                <li>Teste de hipóteses (t-Student, chi-quadrado, etc)</li>
                <li>Regressão linear e não-linear</li>
                <li>ANOVA</li>
                <li>Comparações múltiplas</li>
                <li>Séries temporais</li>
                <li>Machine Learning</li>
                <li>Relatórios dinâmicos (RMarkdown, Quarto)</li>
                <li>Apps e dashboards em Shiny</li>
                <li>Muitos outros!>
            </ul>
        </div>
    </div>
    <div>
    </div>
</div>